...
rem ECF Remote Services demo - CONSUMER process.
rem Imports IRemoteGreet via the static EDEF file and invokes greet("ECF").
rem Start run-ecf-host.bat FIRST, then this in another terminal.
rem Interactive: type `ecf:greet <name>` at the osgi> prompt.

set "PRODUCT_UID=com.kk.pde.ds.ecf.consumer.product"
